        ...
        <div class="card-header">
            <h3 class="card-title align-items-start flex-column mb-1">
                <span class="card-label fw-bolder fs-3 mb-1">
                    <i class="fas fa-list fa-fw mx-2"></i>قضايا قيد الانتظار
                </span>
            ...
            <div class="table-responsive">
                <table class="table align-middle gs-0 gy-4">
                    ...
                    <tbody>
                       ...
                       <td>{{'1'}}</td>
                                <td>
                                    @if($req->additional_file == '' || $req->additional_file == null)
                                ...
                                <td class="text-center">
                                    <span class="text-dark fw-bolder mb-1 fs-6">
                                        @if($req->husband_complate_info == 0)
                                            <i class="fas fa-times fa-fw mx-1 text-danger"></i>
                                        @else
                                            <i class="fas fa-check fa-fw mx-1 text-success"></i>
                                        @endif
                                        @if($req->request_info != '')
                                            {{$req->request_info->husband_name}}
                                        @endif
                                    </span>
                                    <span class="text-muted fw-bold text-muted d-block fs-7">
                                        @if($req->request_info != '')
                                            <a href="tel:{{shortPhone($req->request_info->husband_phone)}}">{{shortPhone($req->request_info->husband_phone)}}</a>
                                        @endif
                                    </span>
                                    @if($req->husband_complate_info == 0)
                                ...
                                <td class="text-center">
                                    <span class="text-dark fw-bolder mb-1 fs-6">
                                        @if($req->wife_complate_info == 0)
                                            <i class="fas fa-times fa-fw mx-1 text-danger"></i>
                                        @else
                                            <i class="fas fa-check fa-fw mx-1 text-success"></i>
                                        @endif
                                        @if($req->request_info != '')
                                            {{$req->request_info->wife_name}}
                                        @endif
                                    </span>
                                    <span class="text-muted fw-bold text-muted d-block fs-7">
                                        @if($req->request_info != '')
                                            <a href="tel:{{shortPhone($req->request_info->wife_phone)}}">{{shortPhone($req->request_info->wife_phone)}}</a>
                                        @endif
                                    </span>
                                    @if($req->wife_complate_info == 0)
                                        <a href="#" class="badge badge-warning">ارسال تذكير</a>
                                ...
                                <td>{!! getRequestStatus($req->status) !!}</td>
                                <td class="text-center">
                                    @if($req->transaction_number != 0)
